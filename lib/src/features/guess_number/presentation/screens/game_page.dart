...
class _GamePageState extends State<GamePage> {
    ...
    return Scaffold(
      ...
      body: BlocConsumer<GamePageBloc, GameState>(
        listener: (context, state) {
          state.maybeWhen(
            won: (targetNumber, maxNumber, remainingAttempts) {
              showDialog(
                context: context,
            ...
            lost: (targetNumber, maxNumber, remainingAttempts) {
              showDialog(
                context: context,
                builder: (_) => ResultDialog(
                  title: 'Игра окончена',
                  content:
                  'Вы проиграли. Загаданное число было: $targetNumber',
                  color: Colors.red,
                  onRestart: () {
                    context.read<GamePageBloc>().add(const ResetGameEvent());
                  },
        ...
        builder: (context, state) {
          return state.maybeMap(
            initial: (_) => _buildInitialInput(context),
            inProgress: (inProgressState) =>
                _buildInProgress(context, inProgressState),
        ...
        children: [
          ...
          GnButton(
            ...
            onPressed: () {
              ...
              final maxAttempts = int.tryParse(_maxAttemptController.text);
  ...
  Widget _buildInProgress(
      BuildContext context, InProgressState inProgressState) {
    return Padding(
      padding: const EdgeInsets.all(16.0),
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
